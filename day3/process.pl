...
foreach my $wire_input (@{$wire_inputs})
{
print STDERR "Tracing Wire\n";
	push @{$wire_coordinates}, trace_wire($wire_input);
print STDERR "Wire Traced\n";
print STDERR "Wire Length: " . (scalar @{$wire_coordinates->[$#{$wire_coordinates}]}) . "\n";
}

print STDERR "Detecting Intersections\n";
my $intersections = detect_intersections($wire_coordinates->[1], $wire_coordinates->[0]);
print STDERR "Intersections Detected\n";

my $nearest = 9999999999999;

foreach my $c (@{$intersections})
{
	my $distance = abs($c->[0]) + abs($c->[1]);
...
{	
	my $distance = 
		distance_to_coordinates($wire_coordinates->[0], $c)
		+ distance_to_coordinates($wire_coordinates->[1], $c);
	$nearest = $distance if $distance < $nearest;
}

print "Steps: $nearest\n";

sub distance_to_coordinates
{
	my ($path, $coordinates) = @_;

	my $d = path_contains_coordinates($path, $coordinates);
	return $d+1;
}


sub detect_intersections
{
	my ($wire1, $wire2) = @_;
	...
	{
		$i++; print STDERR "$i\r";
		push @{$intersections}, $c if path_contains_coordinates($wire2, $c);
	}
	print "\n";
	return $intersections;
}

sub path_contains_coordinates
{
	my ($path, $coordinates) = @_;
	...
	{
		my $c = $path->[$i];
		return $i if coordinates_equal($c, $coordinates);
		$i += manhatten_distance($c, $coordinates);
	}

	return 0;
}

sub coordinates_equal
{
	my ($c1, $c2) = @_;

	return 1 if ($c1->[0] == $c2->[0] && $c1->[1] == $c2->[1]);
	return 0;
...
#	record_position($path, $position);

	my @directions = split(/,/, $wire_input);

	foreach my $d (@directions)
	{
		my $line = direction_to_line($position,$d);

		$position = $line->[$#{$line}];

		record_position($path, $_) foreach @{$line};
	}
	return $path;
}

#does not add the start position, only every subsequent one
sub direction_to_line
{
	...
	if ($direction =~ m/([UDLR])([0-9]+)/)
	{
		...
		foreach (1..$count)
		{
			$position = add_coordinate_offset($position, $direction_offsets->{$dir});
			record_position($line, $position);
		}

		return $line;
	}
	print STDERR "Couldn't parse $direction\n";
}

sub add_coordinate_offset
{
	my ($position, $offset) = @_;

	my $new_position = [];

	push @{$new_position}, $position->[0] + $offset->[0];
	push @{$new_position}, $position->[1] + $offset->[1];
...
sub record_position
{
	...
	push @{$path}, [$position->[0], $position->[1]]; 
}

sub manhatten_distance
{
	my ($c1, $c2) = @_;
